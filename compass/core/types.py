...
class CrRNACandidate(BaseModel):
    ...
    ref_base_at_mutation: Optional[str] = None          # WT base at mutation_position_in_spacer (spacer orientation)
    gc_content: float
    homopolymer_max: int
    ...
    wt_spacer_seq: Optional[str] = None              # wild-type spacer (set by mismatch generation)
...
class ScoredCandidate(BaseModel):
    ...
    cnn_score: Optional[float] = None  # SeqCNN predicted activity (0-1)
    cnn_calibrated: Optional[float] = None  # Temperature-scaled CNN score
    ensemble_score: Optional[float] = None  # alpha*heuristic + (1-alpha)*calibrated
...
class RPAPrimer(BaseModel):
    ...
    RPA primers are 25-38 nt, Tm 57-72°C, and must avoid
    ...
    """
    seq: str = Field(min_length=20, max_length=45)
    tm: float
...
class PanelMember(BaseModel):
    ...
    has_sm: bool = False
    sm_enhanced_spacer: Optional[str] = None        # spacer with SM applied
    sm_position: Optional[int] = None               # position in spacer where SM introduced
    sm_original_base: Optional[str] = None          # original base at SM position
    ...
    asrpa_primers: Optional[RPAPrimerPair] = None   # AS-RPA primers for Proximity targets in shared loci
