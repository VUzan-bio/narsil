...
from guard.core.constants import FLANKING_WINDOW, H37RV_ACCESSION
from guard.core.types import Mutation, Strand, Target
...
class TargetResolver:
    ...
    def _find_gene(self, gene_name: str) -> Optional[GeneRecord]:
        ...
        # Known aliases (gene name → alternative names in GFF3)
        # NOTE: fabG1 promoter mutations (c.-15C>T) must resolve to fabG1,
        # NOT inhA. The operon is fabG1→inhA, and the -15 position is
        # relative to fabG1's start codon.
        ...
        aliases = ALIASES.get(gene_name, [])
        for alias in aliases:
            if alias in self.gene_table:
                return self.gene_table[alias]

        # Search by locus_tag across all genes
        for name, rec in self.gene_table.items():
            if rec.locus_tag and rec.locus_tag.lower() == gene_name.lower():
                return rec
