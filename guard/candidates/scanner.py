...
Self-contained: no imports from guard.core.constants for PAM logic.
...
def snp_positions(target: Target) -> list[int]:
    """Flanking-seq positions (0-based) where ref and alt actually differ.

    For a codon like TCG→TTG, only position 1 differs. For rRNA A→G,
    the single position differs. For deletions ("---"), returns the
    full codon footprint. This prevents false-direct classification
    when a spacer overlaps the codon but not the changed nucleotide.
    """
    offset = target.genomic_pos - target.flanking_start
    ref = target.ref_codon.upper()
    alt = target.alt_codon.upper()

    if ref == "---" or alt == "---" or len(ref) != len(alt):
        # Indel/deletion: whole footprint counts
        size = 3 if ref == "---" else len(ref)
        return list(range(offset, offset + size))

    return [offset + i for i in range(len(ref)) if ref[i] != alt[i]]
...
class ScanResult:
    ...
    positions_scanned: int = 0                      # total positions checked (both strands)
    pam_hits: int = 0                               # positions matching a PAM pattern
...
class PAMScanner:
    ...
    def __init__(
        ...
        # Position counters (reset per scan_detailed call)
    ...
    def scan_detailed(self, target: Target) -> ScanResult:
        ...
        # Save original (reference) flanking BEFORE mutation injection.
        # Used to determine the ref base at each mutation position in the spacer.
        flanking_ref = flanking

        # Inject mutation into flanking so spacers contain the mutant allele.
        # Module 5.5 (MismatchGenerator) expects spacer_seq to be the MUT
        # spacer and derives WT by reverting to ref_codon. Without this,
        # spacers are WT → discrimination ≈ 1× (no mismatch).
        ref_c = target.ref_codon.upper()
        alt_c = target.alt_codon.upper()
        if (
            ref_c != "---"
            and alt_c != "---"
            and len(ref_c) == len(alt_c)
            and 0 <= ms < me <= flen
        ):
            # Detect minus-strand genes: flanking is plus-strand,
            # but ref_codon/alt_codon are in coding orientation.
            # For minus-strand genes, flanking[ms:me] == RC(ref_codon).
            flanking_codon = flanking[ms:me]
            if flanking_codon == ref_c:
                # Plus-strand gene: inject coding alt directly
                alt_inject = alt_c
            elif flanking_codon == str(Seq(ref_c).reverse_complement()):
                # Minus-strand gene: inject RC(alt) to match plus-strand
                alt_inject = str(Seq(alt_c).reverse_complement())
            else:
                # Best effort fallback
                alt_inject = alt_c
                logger.debug(
                    "%s: flanking codon %s doesn't match ref %s or RC(%s)=%s",
                    target.label, flanking_codon, ref_c, ref_c,
                    str(Seq(ref_c).reverse_complement()),
                )

            flanking = flanking[:ms] + alt_inject + flanking[me:]

        # ── Phase 1: Direct overlap scan ──
        direct: list[CrRNACandidate] = []
        seen: set[str] = set()

        # Compute actual SNP nucleotide positions (not full codon footprint).
        # For minus-strand genes, compare plus-strand codons (from flanking)
        # to find which plus-strand positions actually change.
        flanking_ref_codon = flanking_ref[ms:me]
        flanking_alt_codon = flanking[ms:me]
        snp_pos_plus = [
            ms + i
            for i in range(min(len(flanking_ref_codon), len(flanking_alt_codon)))
            if flanking_ref_codon[i] != flanking_alt_codon[i]
        ]

        logger.debug(
            "%s: ref=%s alt=%s footprint=[%d,%d) snp_positions=%s",
            target.label, ref_c, alt_c, ms, me, snp_pos_plus,
        )

        # Plus strand
        self._scan_strand_direct(
            target, flanking, Strand.PLUS, ms, me, flen, direct, seen,
            snp_pos_plus, flanking_ref,
        )

        # Minus strand — convert SNP positions to RC coordinates
        rc = str(Seq(flanking).reverse_complement())
        rc_ref = str(Seq(flanking_ref).reverse_complement())
        snp_pos_minus = sorted(flen - 1 - p for p in snp_pos_plus)
        self._scan_strand_direct(
            target, rc, Strand.MINUS, flen - me, flen - ms, flen, direct, seen,
            snp_pos_minus, rc_ref,
        )
        ...
        # Trigger proximity if no direct candidates OR if all direct
        # candidates have the mutation outside the seed region (PAM-distal
        # mismatches give ~1× discrimination with Cas12a).
        all_outside_seed = (
            len(direct) > 0
            and all(
                c.mutation_position_in_spacer is None
                or c.mutation_position_in_spacer > self.config.seed_end
                for c in direct
            )
        )

        if (len(direct) == 0 or all_outside_seed) and self.proximity.enabled:
            is_desert = len(direct) == 0
            reason = "PAM DESERT" if is_desert else "SEED MISS (all direct candidates have mutation outside seed)"
            logger.info(
                "%s for %s — activating proximity scan "
        ...
        return ScanResult(
            ...
            positions_scanned=self._positions_scanned,
            pam_hits=self._pam_hits,
        )
    ...
    def _scan_strand_direct(
        ...
        seen: set[str],
        changed_positions: Optional[list[int]] = None,
        ref_seq: Optional[str] = None,
    ) -> None:
        """Scan one strand for PAMs with spacers that overlap the mutation.

        When changed_positions is provided, only count overlap with
        nucleotides that actually differ between ref and alt codons.
        This prevents false-direct classification when a spacer overlaps
        the codon footprint but not the changed nucleotide.
        """
        slen = len(seq)
        ...
        for i in range(slen - 4):
            ...
            for sp_len in self.lengths:
                ...
                # Overlap check: spacer must contain at least one
                # actually-changed nucleotide (not just the codon footprint)
                if changed_positions:
                    snp_in_spacer = [
                        p for p in changed_positions
                        if sp_start <= p < sp_end
                    ]
                    if not snp_in_spacer:
                        continue
                else:
                    # Fallback: use full codon footprint
                    if max(sp_start, ms) >= min(sp_end, me):
                        continue

                # Mutation positions in spacer (1-indexed, PAM-proximal)
                # Use changed positions if available, else full footprint
                check_positions = changed_positions if changed_positions else list(range(ms, me))
                positions = [
                    p - sp_start + 1
                ...
                # Determine the ref (WT) base at the mutation position
                # from the original (pre-injection) sequence
                ref_base = None
                if mut_pos is not None and ref_seq is not None:
                    ref_idx = sp_start + mut_pos - 1  # 0-indexed in seq
