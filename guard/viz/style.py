"""Shared style for all GUARD figures.
...
CMAP_SEQ = mpl.colors.LinearSegmentedColormap.from_list(
    "guard_seq", [PALETTE["heatmap_low"], PALETTE["light_blue"], PALETTE["blue"]]
)

# Diverging colourmap for discrimination (blue = low activity, red = high)
CMAP_DIV = mpl.colors.LinearSegmentedColormap.from_list(
    "guard_div", [PALETTE["diverging_low"], PALETTE["diverging_mid"], PALETTE["diverging_high"]]
)
...
def apply_style() -> None:
    """Apply GUARD publication style to all matplotlib figures."""
    mpl.rcParams.update({
        # Font
